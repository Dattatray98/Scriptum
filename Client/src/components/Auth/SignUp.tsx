...
const SignUp: React.FC<any> = ({ setLoginOpen, LoginOpen }) => {
  ...
  return (
    <div className={`fixed border h-full w-full flex justify-center items-center z-40  inset-0 bg-black/20 backdrop-blur-lg`}>
      <motion.div
        initial={{ scale: 0.50, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.50, opacity: 0 }}
        transition={{ duration: 0.3, ease: "easeInOut" }}
        className={`border border-white/50 shadow-2xl w-[100vh] h-[60vh] relative  rounded-4xl bg-white/10 backdrop-blur-3xl ${LoginOpen ? "scale-100" : ""}`}
      >
        ...
        <div className=" absolute right-5 top-5 flex justify-end z-60">
          <RxCross2 onClick={() => setLoginOpen(!LoginOpen)} className="h-5 w-5 text-red-500 cursor-pointer" />
        </div>
            ...
            {!IsSignUP ? (
              <div className="flex flex-col h-full z-55 items-center justify-center p-4 gap-8 mb-6">
                ...
                <div className="flex gap-2">
                  <p className="font-medium text-gray-300">Don't have an Account?</p>
                  <p className="font-medium text-cyan-400 hover:text-blue-400 cursor-pointer transition-all duration-300" onClick={handleSignSwitch}>Create New.</p>
                </div>
              </div>
            ) : (
          ...
          <div className="w-[50%] h-full rounded-bl-4xl z-55 ">
            {IsSignUP ? (
              <div className="flex flex-col h-full w-full z-55 items-center justify-center p-4 gap-8 mb-6">
                ...
                <div className="flex gap-2">
                  <p className="font-medium text-gray-300">Already have an Account?</p>
